...
-- pg -d $DATABASE_URL -c "CREATE EXTENSION postgis; set client_encoding to 'UTF8';"
...
-- cat  ~/Desktop/beers.csv | pg -d $DATABASE_URL -c "\copy beers (id, name, brewery_id, style_id, ba_link, ba_availability, ba_description, abv, rating_counts, total_score, beer_flavor_wheel, review_text_json) FROM STDIN WITH (FORMAT CSV, HEADER TRUE);"
-- cat  ~/Desktop/breweries.csv | pg -d $DATABASE_URL -c "\copy breweries (id, name, phone_number, address, zipcode, city, state, country, website, position, features, ba_link) FROM STDIN WITH (FORMAT CSV, HEADER TRUE);"
-- cat  ~/Desktop/styles.csv | pg -d $DATABASE_URL -c "\copy styles ( id, name, ba_link, abv_range, ibu_range, glassware, description) FROM STDIN WITH (FORMAT CSV, HEADER TRUE);"
